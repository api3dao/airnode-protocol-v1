  ...
  describe('externalMulticallWithValue', function () {
      ...
      context('Value sent with the call is sufficient', function () {
          ...
          context('None of the calls reverts', function () {
            context('Call does not have any excess value', function () {
              it('multicall does not revert', async function () {
                const { oevSearcherMulticall, multicallTargets } = await helpers.loadFixture(deploy);
                ...
                const returndata = await oevSearcherMulticall.callStatic.externalMulticallWithValue(
                  targets,
                  data,
                  values,
                  {
                ...
                await expect(
                  oevSearcherMulticall.externalMulticallWithValue(targets, data, values, { value: totalValue })
                ).to.not.be.reverted;
        ...
        context('Value sent with the call is insufficient', function () {
          it('multicall reverts', async function () {
            ...
            const values = [100, 200, 300];
            await expect(
              oevSearcherMulticall.externalMulticallWithValue(targets, data, values, { value: 100 })
    ...
    context('Call have excess value', function () {
      it('multicall reverts', async function () {
        ...
        const totalValue = values.reduce((a, b) => a + b, 0);
        await expect(
          oevSearcherMulticall.externalMulticallWithValue(targets, data, values, { value: totalValue + 1 })
