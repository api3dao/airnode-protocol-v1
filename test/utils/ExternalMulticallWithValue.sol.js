...
describe.only('ExternalMulticallWithValue', function () {
  async function deploy() {
    const accounts = await ethers.getSigners();
    const roles = {
      deployer: accounts[0],
    };
    const ExternalMulticallWithValueFactory = await ethers.getContractFactory('MockExternalMulticallWithValue', roles.deployer);
        ...
        context('None of the calls is to a non-contract account', function () {
          context('None of the calls reverts', function () {
            ...
            it('multicall does not revert', async function () {
              ...
              const returndata = await externalMulticallWithValue.callStatic.externalMulticallWithValue(targets, data, values, {
                value: totalValue,
              });
          ...
          context('Calls multiple separate contracts', function (){
            it('multicall does not revert', async function () {
              const { externalMulticallWithValue, multicallTarget, MockMulticallTargetFactory } = await helpers.loadFixture(deploy);
          ...
          context('One of the calls reverts', function () {
            context('Call reverts with string', function () {
              it('multicall reverts by bubbling up the revert string', async function () {
                ...
                const values = [0, 0, 0];
                const totalValue = values.reduce((a, b) => a + b, 0);
                await expect(externalMulticallWithValue.externalMulticallWithValue(targets, data, values, { value: totalValue })).to.be.revertedWith(
            ...
            context('Call reverts with custom error', function () {
              it('multicall reverts by bubbling up the custom error', async function () {
                ...
                const values = [0, 0, 0];
                const totalValue = values.reduce((a, b) => a + b, 0);
                await expect(externalMulticallWithValue.externalMulticallWithValue(targets, data, values, { value: totalValue })).to.be.revertedWithCustomError(
                  multicallTarget,
                  'MyError'
                );
              });
