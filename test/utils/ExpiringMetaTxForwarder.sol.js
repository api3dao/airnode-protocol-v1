...
describe('ExpiringMetaTxForwarder', function () {
  ...
  describe('multicall', function () {
    it('executes multiple meta-txes', async function () {
      const {
        roles,
        expiringMetaTxForwarder,
        expiringMetaTxForwarderTarget,
        expiringMetaTxDomain,
        expiringMetaTxTypes,
        expiringMetaTxValue: expiringMetaTxValue1,
      } = await helpers.loadFixture(deploy);
      // Value can't be identical
      const expiringMetaTxValue2 = {
        ...expiringMetaTxValue1,
        expirationTimestamp: expiringMetaTxValue1.expirationTimestamp + 1,
      };
      ...
      const signature2 = await roles.owner._signTypedData(
        expiringMetaTxDomain,
        expiringMetaTxTypes,
        expiringMetaTxValue2
      );
      const counterInitial = await expiringMetaTxForwarderTarget.counter();
      const metaTxTypedDataHash1 = deriveTypedDataHashOfMetaTx(expiringMetaTxDomain, expiringMetaTxValue1);
      const metaTxTypedDataHash2 = deriveTypedDataHashOfMetaTx(expiringMetaTxDomain, expiringMetaTxValue2);
      expect(await expiringMetaTxForwarder.metaTxWithHashIsExecuted(metaTxTypedDataHash1)).to.equal(false);
      expect(await expiringMetaTxForwarder.metaTxWithHashIsExecuted(metaTxTypedDataHash2)).to.equal(false);
      await expect(
        expiringMetaTxForwarder
          .connect(roles.randomPerson)
          .multicall([
            expiringMetaTxForwarder.interface.encodeFunctionData('execute', [expiringMetaTxValue1, signature1]),
            expiringMetaTxForwarder.interface.encodeFunctionData('execute', [expiringMetaTxValue2, signature2]),
          ])
      ).to.not.be.reverted;
      expect(await expiringMetaTxForwarderTarget.counter()).to.be.equal(counterInitial.add(2));
      expect(await expiringMetaTxForwarder.metaTxWithHashIsExecuted(metaTxTypedDataHash1)).to.equal(true);
      expect(await expiringMetaTxForwarder.metaTxWithHashIsExecuted(metaTxTypedDataHash2)).to.equal(true);
    });
  });

  describe('tryMulticall', function () {
    it('tries to execute multiple meta-txes', async function () {
      const {
        roles,
        expiringMetaTxForwarder,
        expiringMetaTxForwarderTarget,
