...
describe('Api3ServerV1', function () {
  ...
  async function encodeUpdateSubscriptionConditions(
    ...
    return ethers.utils.defaultAbiCoder.encode(
      ...
      [
        ...
        (await api3ServerV1.provider.getNetwork()).chainId,
  ...
  async function deploy() {
    ...
    const beaconSetUpdateSubscriptionConditions = await encodeUpdateSubscriptionConditions(
      api3ServerV1,
      api3ServerV1.interface.getSighash('conditionPspBeaconSetUpdate'),
    ...
    const beaconSetUpdateSubscriptionId = await deriveUpdateSubscriptionId(
      ...
      beacons[0].airnode.wallet.address,
      roles.sponsor.address,
      api3ServerV1.interface.getSighash('fulfillPspBeaconSetUpdate'),
  ...
  describe('constructor', function () {
    it('constructs', async function () {
      ...
      expect(await api3ServerV1.HUNDRED_PERCENT()).to.equal(HUNDRED_PERCENT);
            ...
            context('Update is upwards', function () {
              context('It has been at least heartbeat interval seconds since the last update', function () {
                it('returns true', async function () {
                  ...
                  await updateBeaconSet(roles, api3ServerV1, beacons, 100);
                  ...
                  await Promise.all(
                    beacons.map(async (beacon) => {
                      await updateBeacon(roles, api3ServerV1, beacon, 110);
                    })
                  );
                  expect(
                    await api3ServerV1
            ...
            context('Update is downwards', function () {
              context('It has been at least heartbeat interval seconds since the last update', function () {
                it('returns true', async function () {
                  ...
                  await Promise.all(
                    beacons.map(async (beacon) => {
                      await updateBeacon(roles, api3ServerV1, beacon, 90);
                    })
                  );
                  expect(
                    await api3ServerV1
            ...
            context('Update is upwards', function () {
              context('It has been at least heartbeat interval seconds since the last update', function () {
                it('returns true', async function () {
                  ...
                  await Promise.all(
                    beacons.map(async (beacon) => {
                      await updateBeacon(roles, api3ServerV1, beacon, 109);
                    })
                  );
                  expect(
                    await api3ServerV1
            ...
            context('Update is downwards', function () {
              context('It has been at least heartbeat interval seconds since the last update', function () {
                it('returns true', async function () {
                  ...
                  await Promise.all(
                    beacons.map(async (beacon) => {
                      await updateBeacon(roles, api3ServerV1, beacon, 91);
                    })
                  );
                  expect(
                    await api3ServerV1
